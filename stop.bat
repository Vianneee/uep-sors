...
:: Find and kill process on port 8080 (main backend)
echo Stopping process on port 8080...
for /f "tokens=5" %%a in ('netstat -aon ^| findstr /r "LISTENING" ^| findstr ":8080"') do (
    taskkill /f /pid %%a >nul 2>nul
)

:: Find and kill process on port 8082 (organizations)
echo Stopping process on port 8082...
for /f "tokens=5" %%a in ('netstat -aon ^| findstr /r "LISTENING" ^| findstr ":8082"') do (
    taskkill /f /pid %%a >nul 2>nul
)
